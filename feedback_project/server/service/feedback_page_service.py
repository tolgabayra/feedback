...
class FeedbackPageService:
    ...
    def create(id):
        ...
        feedback_page = BusinessFeedbackPage(
            url_token=url_token, expire_time=expire_time, business_id=id
        )
    ...
    @staticmethod
    def show(url_token):
        feedback_page = BusinessFeedbackPage.query.filter_by(
            url_token=url_token
        ).first()
    ...
    def list(business_id):
        feedback_pages = BusinessFeedbackPage.query.filter_by(
    ...
    def list_types():
        ...
        types_list = []
        for type in types:
            types_list.append({"id": type.id, "name": type.name})
        return types_list
